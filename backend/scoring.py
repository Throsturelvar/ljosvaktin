...
sources.py hefur sótt. Vægi: virkni 35%, Kp-leitni 10%, heiðskírt 40%,
tunglbirta 15% - skýjahula vegur þyngst því norðurljós sjást einfaldlega
ekki gegnum ský, óháð virkni."""
...
def reikna_skor(virkni, kp, sky_pct, tungl_pct, myrkur_fra, myrkur_til):
    ...
    virkni_stig = min((virkni or 0) / 14.0, 1.0) * 100
    kp_stig = min((kp or 0) / 9.0, 1.0) * 100 if kp is not None else virkni_stig
    ...
    return {
        ...
        "sundurlidun": {
            "virkni_stig": round(virkni_stig, 1),
